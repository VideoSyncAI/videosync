...
path_to_dataset = '/home/username/koa_scratch/penn_action'
...
class PennAction(torch.utils.data.Dataset):
    def __init__(self, cfg, split, dataset_name=None, mode="auto", sample_all=False):
        ...
        if dataset_name is not None:
            indices = self.action_to_indices[PENN_ACTION_LIST.index(
                dataset_name)]
            self.dataset = [self.dataset[index] for index in indices]
            logger.info(
                f"{len(self.dataset)} {self.split} samples of {dataset_name} dataset have been read.")
        else:
            logger.info(
                f"{len(self.dataset)} {self.split} samples of Penn Action dataset have been read.")
        ...
        for i in iter(range(self.num_samples)):
            if action == -1:
                action = torch.randint(
                    high=len(self.action_to_indices), size=(1,), dtype=torch.int64)
